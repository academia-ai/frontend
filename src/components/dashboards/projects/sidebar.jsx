...
const ProjectSidebar = ({data,  selectedContent, handleSelectContent,setChapNum }) => {
...
<div className='flex flex-col w-full gap-2'>
            <h1 className='font-semibold text-zinc-300 text-base
             md:text-xl'>{data?.title ?? ''}</h1>
               <p className="text-zinc-400  md:text-xs max-w-2xl"> {data.desc ?? ''}</p>
...
<div className='flex-1 overflow-y-scroll max-h-[60vh] px-1  '>
            ...
            <div>
                 ...
                 <div 
onClick={() => handleSelectContent(data?.certification, 'Certification')}
      ...
      {data?.chapters?.map((chapter,index) => (
        <div onClick={() =>{
          ...
          className={`flex items-center gap-2 px-3 py-3 rounded-md cursor-pointer relative group
          hover:bg-zinc-800/40 transition-all text-zinc-400 shadow-lg shadow-zinc-900
          ${selectedContent?.title === chapter?.title ? "bg-zinc-800" : "bg-zinc-900"}`}
        >
          <GripVertical size={14} className="text-zinc-500" />
